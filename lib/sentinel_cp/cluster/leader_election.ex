defmodule SentinelCp.Cluster.LeaderElection do
  @moduledoc """
  ...
  defp detect_adapter do
    case Application.get_env(:sentinel_cp, :ecto_adapter) do
  ...
  defp try_acquire_lock do
    case SentinelCp.Repo.query("SELECT pg_try_advisory_lock($1)", [@lock_id]) do
      {:ok, %{rows: [[true]]}} -> true
      _ -> false
    end
  rescue
