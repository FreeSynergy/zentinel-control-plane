defmodule SentinelCp.Cluster.Backup do
  @moduledoc """
  ...
  def system_inventory do
    %{
      ...
      cluster_info: SentinelCp.Cluster.Health.cluster_info()
    }
  end
  ...
  def export_all_projects do
    from(p in SentinelCp.Projects.Project, order_by: [asc: p.name])
    |> Repo.all()
    |> Enum.map(fn project ->
      case SentinelCp.ConfigExport.export(project.id) do
  ...
  def generate_runbook do
    ...
    """
    # Sentinel CP Disaster Recovery Runbook
