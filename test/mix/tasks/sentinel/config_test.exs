defmodule Mix.Tasks.Sentinel.ConfigTest do
  ...
  describe "sentinel.config.export" do
    ...
    test "exports to file with --output", %{project: project} do
      ...
      capture_io(fn ->
        Mix.Tasks.Sentinel.Config.Export.run([project.slug, "--output", path])
      end)
  ...
  describe "sentinel.config.apply" do
    ...
    test "shows diff without modifying with --dry-run", %{project: project} do
      ...
      envs = Repo.all(SentinelCp.Projects.Environment)
  ...
  describe "sentinel.config.diff" do
    ...
    test "shows no differences for matching config", %{project: project} do
      {:ok, current} = SentinelCp.ConfigExport.export(project.id)
