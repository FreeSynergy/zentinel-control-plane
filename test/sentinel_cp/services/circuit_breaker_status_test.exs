defmodule SentinelCp.Services.CircuitBreakerStatusTest do
  use SentinelCp.DataCase

  alias SentinelCp.Services
  alias SentinelCp.Services.CircuitBreakerStatus

  defp create_project_node_and_group(_) do
    {:ok, org} = SentinelCp.Orgs.create_org(%{name: "CB Org", slug: "cb-org"})

    {:ok, project} =
      SentinelCp.Projects.create_project(%{name: "CB Project", slug: "cb-proj", org_id: org.id})
