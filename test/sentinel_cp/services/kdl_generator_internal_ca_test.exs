defmodule SentinelCp.Services.KdlGeneratorInternalCaTest do
  use ExUnit.Case, async: true

  alias SentinelCp.Services.KdlGenerator
  alias SentinelCp.Services.{ProjectConfig, InternalCa, AuthPolicy}
  ...
  describe "client_auth block" do
    test "includes client_auth block when internal CA exists" do
      ...
      assert kdl =~ ~s(ca_file "/etc/sentinel/internal-ca/ca.pem")
      assert kdl =~ ~s(crl_file "/etc/sentinel/internal-ca/crl.pem")
    end
