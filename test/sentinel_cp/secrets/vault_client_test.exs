defmodule SentinelCp.Secrets.VaultClientTest do
  ...
  describe "list_secrets/2" do
    test "returns list of keys" do
      ...
      assert {:ok, ["db-password", "api-key", "jwt-secret"]} =
               SentinelCp.Secrets.VaultClient.Mock.list_secrets(
                 %{vault_addr: "http://vault:8200"},
                 "apps/"
               )
    end
  ...
  describe "health/1" do
    test "returns health status" do
      ...
      assert {:ok, %{initialized: true, sealed: false}} =
               SentinelCp.Secrets.VaultClient.Mock.health(%{vault_addr: "http://vault:8200"})
    end
  end
end
