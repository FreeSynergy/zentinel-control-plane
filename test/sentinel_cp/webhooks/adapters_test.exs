defmodule SentinelCp.Webhooks.AdaptersTest do
  use ExUnit.Case, async: true

  alias SentinelCp.Webhooks.{GitLabAdapter, BitbucketAdapter, GiteaAdapter, GenericAdapter}
  ...
  describe "GiteaAdapter" do
    ...
    test "parses push event (GitHub-compatible format)" do
      payload = %{
        ...
        "repository" => %{"full_name" => "org/repo"},
        "commits" => [
          %{"added" => ["sentinel.kdl"], "modified" => [], "removed" => []}
        ]
      }
      ...
      assert result.branch == "main"
      assert result.commit == "abc123"
      assert "sentinel.kdl" in result.commits
    end
  end
